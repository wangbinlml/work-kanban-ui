    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['board:cardActivity:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['board:cardActivity:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['board:cardActivity:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['board:cardActivity:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cardActivityList" @selection-change="handleSelectionChange">
...
<script>
import { listCardActivity, getCardActivity, delCardActivity, addCardActivity, updateCardActivity } from "@/api/board/cardActivity";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCardActivity(this.queryParams).then(response => {
        this.cardActivityList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getCardActivity(id).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateCardActivity(this.form).then(response => {
            ...
            addCardActivity(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除卡片活动关系编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delCardActivity(ids);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('board/cardActivity/export', {
        ...this.queryParams
      }, `cardActivity_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
