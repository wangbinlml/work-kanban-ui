    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['board:cardAttachment:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['board:cardAttachment:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['board:cardAttachment:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['board:cardAttachment:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cardAttachmentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="附件类型(1：图片，2：文件)" align="center" prop="type" />
...
<script>
import { listCardAttachment, getCardAttachment, delCardAttachment, addCardAttachment, updateCardAttachment } from "@/api/board/cardAttachment";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        type: [
          { required: true, message: "附件类型(1：图片，2：文件)不能为空", trigger: "change" }
        ],
        url: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCardAttachment(this.queryParams).then(response => {
        this.cardAttachmentList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getCardAttachment(id).then(response => {
        this.loading = false;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateCardAttachment(this.form).then(response => {
            ...
            addCardAttachment(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除卡片附件关系编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delCardAttachment(ids);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('board/cardAttachment/export', {
        ...this.queryParams
      }, `cardAttachment_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
