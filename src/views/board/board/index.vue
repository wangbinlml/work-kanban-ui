<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="boardList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="可见范围" align="center" prop="viewRange">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.view_range" :value="scope.row.viewRange"/>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除看版管理编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delBoard(ids);
      }).then(() => {
        this.loading = false;
