<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="cardList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="背景图片" align="center" prop="backgroundImage" />
...
<script>
import { listCard, getCard, delCard, addCard, updateCard } from "@/api/board/card";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除卡片编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delCard(ids);
      }).then(() => {
        this.loading = false;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('board/card/export', {
        ...this.queryParams
