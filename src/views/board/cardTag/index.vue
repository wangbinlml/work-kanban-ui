    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['board:cardTag:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['board:cardTag:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="cardTagList" @selection-change="handleSelectionChange">
...
<script>
import { listCardTag, getCardTag, delCardTag, addCardTag, updateCardTag } from "@/api/board/cardTag";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCardTag(this.queryParams).then(response => {
        this.cardTagList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateCardTag(this.form).then(response => {
            ...
            addCardTag(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除卡片标签关系编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delCardTag(ids);
      }).then(() => {
        this.loading = false;
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('board/cardTag/export', {
        ...this.queryParams
      }, `cardTag_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
